...
func (r *scrapingRepository) Save(result *entity.ScrapingResult) error {
	...
	query := `INSERT INTO scraping_results (
		url, title, description, keywords, author, language, favicon, 
		image_url, site_name, links, images, headers, status_code, 
		content_type, word_count, load_time_ms, created_at
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`

	res, err := r.db.Exec(query,
		result.URL, result.Title, result.Description, result.Keywords,
		result.Author, result.Language, result.Favicon, result.ImageURL,
		result.SiteName, string(linksJSON), string(imagesJSON),
